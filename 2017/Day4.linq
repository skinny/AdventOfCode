...
string phrases = @"oaoe rxeq vssdqtu xrk cjv yaoqp loo
...
var validPhrases = (from p in phrases.Split(new[] { Environment.NewLine }, StringSplitOptions.RemoveEmptyEntries)
					where  p.Split(' ').GroupBy(x => x).All(g => g.Count() == 1)
					select p).Count();

$"Part 1 Answer : {validPhrases}".Dump();

validPhrases = (from p in phrases.Split(new[] { Environment.NewLine }, StringSplitOptions.RemoveEmptyEntries)
				where p.Split(' ').GroupBy(word =>new string(word.ToCharArray().OrderBy(l => l).ToArray() )).All(k=>k.Count()==1)
				select p).Count();

$"Part 2 Answer : {validPhrases}".Dump();
